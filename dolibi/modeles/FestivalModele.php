...
class FestivalModele
{
    ...
    {   
        $sql = "DELETE FROM SpectacleScenes WHERE idFestival = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam("id",$idFestival);
        $stmt->execute();

        $sql2 = "DELETE FROM SpectaclesJour WHERE idFestival = :id";
        $stmt2 = $pdo->prepare($sql2);
        $stmt2->bindParam("id",$idFestival);
        $stmt2->execute();

        $sql3 = "DELETE FROM Jour WHERE idGrij = :id";
        $stmt3 = $pdo->prepare($sql3);
        $stmt3->bindParam("id",$idFestival);
        $stmt3->execute();

        $sql4 = "DELETE FROM Grij WHERE idGrij = :id";
        $stmt4 = $pdo->prepare($sql4);
        $stmt4->bindParam("id",$idFestival);
        $stmt4->execute();

        $sql5 = "DELETE FROM EquipeOrganisatrice WHERE idFestival = :id";
        $stmt5 = $pdo->prepare($sql5);
        $stmt5->bindParam("id",$idFestival);
        $stmt5->execute();

        $sql6 ="DELETE FROM SpectacleDeFestival WHERE idFestival = :id";
        $stmt6 = $pdo->prepare($sql6);
        $stmt6->bindParam("id",$idFestival);
        $stmt6->execute();

        $sql7 = "DELETE FROM Festival WHERE idFestival = :id";
        $stmt7 = $pdo->prepare($sql7);
        $stmt7->bindParam("id",$idFestival);
        $stmt7->execute();

        $sql8 = "DELETE FROM Festival WHERE idFestival = :id";
        $stmt8 = $pdo->prepare($sql8);
        $stmt8->bindParam("id",$idFestival);
        $stmt8->execute();
